...
router.delete('/', async function(req, res, next) {
	...
	if (accessToken == null) {
		...
		var del_refresh_token_sql = "delete from user where id = ?";
		...
		if(select_token_result.length==0){	//db에 등록된 리프래쉬 토큰이 없을때 
			...
			const [check_token_result, field1] = await con.query(select_token_sql, select_token_param);
			const [check_user_inform_result, field3] = await con.query(select_user_inform_sql ,select_user_inform_param);
		...
		if(access.success){
			if(refresh.success){	//인증 성공 
				...
				if(update_success){
					...
					res.send({success : "fail", msg : "리프래쉬 재발급 실패 "});
				}
			}
			else{}
		}
			...
			if(refresh.success){	//엑세스 다시발급 
				...
				res.send({token : new_access_token, msg : "액세스 재발급"});
                res.send("액세스 다시 발급");
			}
